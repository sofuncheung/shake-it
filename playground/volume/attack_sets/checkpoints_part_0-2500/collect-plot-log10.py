...
ax.set_xlabel(r'$log_{10}P(f)$',fontdict={'fontsize': 16, 'fontweight': 'medium'})
ax.set_ylabel('Generalization (%)',fontdict={'fontsize': 16, 'fontweight': 'medium'})
ax.tick_params(direction='in')
fig.text(0.5,0.18,'ResNet50/CIFAR-10/\nAdam', bbox=dict(facecolor='none'), fontsize=20)
fig.savefig('volume-generalization-resnet50-prior.png', dpi=300)
plt.clf()

fig, ax = plt.subplots()
ax.scatter(-sharpness,volume)
ax.set_xlabel(r'-$log_{10}$(sharpness)',fontdict={'fontsize': 16, 'fontweight': 'medium'})
ax.set_ylabel(r'$log_{10}P(f)$',fontdict={'fontsize': 16, 'fontweight': 'medium'})
ax.tick_params(direction='in')
